...
let lastDeck = null, lastSlide = null;
function pushDeck() {
  const e = embedFromUrl(location.href);
  if (e && e !== lastDeck) { lastDeck = e; chrome.runtime.sendMessage({ type: "deck", embedBase: e }).catch(() => {}); }
}
function tick() {
  pushDeck();
  const id = currentSlideId();
  if (id && id !== lastSlide) { lastSlide = id; chrome.runtime.sendMessage({ type: "slide", slideId: id }).catch(() => {}); }
}
