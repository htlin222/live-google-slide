...
(() => {
  ...
  const style = h("style", { id: "lgs-style", textContent: `
    ...
    .lgs-warn{color:#d93025;font-size:12px;margin:2px 0 0}.lgs-warn:empty{display:none}
  ...
  const keyI = h("input", { id: "lgs-key", type: "password", placeholder: "presenter key" });
  const adv = h("div", { class: "lgs-adv", hidden: true },
    h("label", { textContent: "Cloudflare 網址（你的 Worker，含 https://）" }), cfI,
    h("label", { textContent: "控制密碼（Worker 的 PRESENT_KEY）" }), keyI,
    h("div", { class: "lgs-hint", textContent: "這兩個值只填一次，之後每份簡報共用。" }));
  const advToggle = h("div", { class: "lgs-adv-toggle", textContent: "⚙ 設定 CF 網址 / 控制密碼" });
  ...
  async function load() {
    ...
    keyI.value = sync.key || "";
    if (!sync.cfUrl || !sync.key) adv.hidden = false;
  ...
  function setActiveUI(active, pin) {
    ...
    [embedI, roomI, pinI, genB, cfI, keyI].forEach(n => (n.disabled = active));
  ...
  async function refreshStatus() {
    ...
    if (local.active) {
      ...
      setText(statusBox, h("small", { textContent: "填好上面、按「開始直播」，再進入放映。" }));
    }
  }
  ...
  function positionPanel() {
    ...
    let left = Math.max(8, Math.min(r.right - 300, window.innerWidth - 308));
  ...
  startB.addEventListener("click", async () => {
    const cfUrl = cfI.value.trim(), key = keyI.value.trim();
    const embedBase = embedI.value.trim();
    const room = roomI.value.trim() || "talk", pin = pinI.value.trim();
    if (!cfUrl || !key) { adv.hidden = false; warn.textContent = "請先填 CF 網址與控制密碼。"; return; }
